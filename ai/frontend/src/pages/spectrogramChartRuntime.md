...
- `createSpectrogramChartRuntime(deps: SpectrogramPageDeps)` [deps: [fetchSpectrogram][1], [createAnalysisPageRuntime][2], [exportEChartsPNG][3], [exportEChartsSVG][4], [exportEChartsHTML][5]]
  - Returns runtime with `mount()`: sets up chart, resize observer, column select listener, time range change handler, drag-to-zoom selection box (pointer down/move/up/cancel + dblclick reset), and export bindings.
