...
- `setScatterView(viewName: string, options?: { render?: boolean }): Promise<void>`
  - Updates `appState.scatter.activeView`, syncs sidebar selection, view-button aria-pressed, and per-view panel visibility. When `options.render !== false`, also calls `renderScatterMatrixView` for matrix mode and resizes the active plot.
