...
| --- | --- | --- |
| Portable plan core | Implemented for `timeRange`, `columnRange`, `adaptiveLine`, column-scoped `missingValue`, stable `deduplicate`, ordered keep/drop `columnSelect`, and `annotation` | Shared TypeScript store/compiler and Rust DTO/compiler exist; advanced row, temporal, robust-cleaning, transform, drift, and frequency stages remain |
