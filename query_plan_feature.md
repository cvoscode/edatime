...
| --- | --- | --- |
| Portable plan core | Implemented for `timeRange`, `columnRange`, `adaptiveLine`, column-scoped `missingValue`, stable `deduplicate`, and `annotation` | Shared TypeScript store/compiler and Rust DTO/compiler exist; advanced row, column, transform, drift, and frequency stages remain |
