...
| --- | --- | --- |
| Portable plan core | Implemented for `timeRange`, `columnRange`, `adaptiveLine`, column-scoped `missingValue`, stable `deduplicate`, ordered keep/drop `columnSelect`, stable `sort`, bounded forward/backward `fillNull`, and `annotation` | Shared TypeScript store/compiler and Rust DTO/compiler exist; advanced row, temporal, robust-cleaning, transform, drift, and frequency stages remain |
...
- Cleaning-plan DTO stage fields deserialize in the same camelCase shape the
  frontend emits, including adaptive-line coordinates and options; malformed
  plan payloads remain rejected by the strict DTO contract.
