...
describe('cleaning code generation', () => {
    it('marks portable code exports as client-side previews', () => {
        ...
        expect(generatePythonPolars(plan).startsWith('# Client-side EdaTime plan preview.\n# Revalidate against the backend-exported canonical plan before execution.')).toBe(true);
        expect(generateRustPolars(plan).startsWith('// Client-side EdaTime plan preview.\n// Revalidate against the backend-exported canonical plan before execution.')).toBe(true);
    });
