import { hasFilterModalOpener, openFilterForColumn } from './filterModalService.js';
...
export function initChartPageFilterGesture(): void {
    ...
    pageChart.addEventListener('contextmenu', (event: MouseEvent) => {
        if ((event.target as HTMLElement)?.closest?.('#main-chart') || !hasFilterModalOpener()) return;
