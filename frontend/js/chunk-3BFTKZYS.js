...
var DEFAULT_SCATTER_SUGGESTION_THRESHOLD = 0.7;
var KDE_SAMPLES = 64;
...
var collectColumnRangeFilters = () => Object.entries(appState.columnRanges || {}).map(([column, range]) => {
  const from = Number(range?.from);
  const to = Number(range?.to);
  if (!column || !Number.isFinite(from) || !Number.isFinite(to)) return null;
  return { column, from, to };
}).filter((f) => f !== null);
var scopeFiltersToColumns = (filters, columns) => {
...
//# sourceMappingURL=chunk-3BFTKZYS.js.map
