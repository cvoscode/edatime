...
const CHART_GRID$1 = { left: 120, right: 30, top: 16, bottom: 36 };
class ChartOverlays {
  _opts;
  constructor(opts) {
    this._opts = opts;
  ...
  _renderRollingBandsToCtx(ctx, scale) {
    const bands = appStateComposite.rollingBands;
    if (!bands || bands.length === 0 || !appStateComposite.rollingEnabled) return;
    ...
    const plotLeft = CHART_GRID$1.left * scale.x;
    const plotTop = CHART_GRID$1.top * scale.y;
    const plotRight = Math.max(plotLeft + 1, (cssWidth - CHART_GRID$1.right) * scale.x);
    const plotBottom = Math.max(plotTop + 1, (cssHeight - CHART_GRID$1.bottom) * scale.y);
  ...
  _renderAnomalyRegionsToCtx(ctx, scale) {
    const regions = appStateComposite.anomalyRegions;
    if (!regions || regions.length === 0 || !appStateComposite.anomalyEnabled) return;
    const container = this._opts.getContainer();
    if (!container) return;
    const xMin = this._opts.getXMin();
    const xMax = this._opts.getXMax();
  ...
  _renderAdaptiveFilterLinesToCtx(ctx, scale) {
    ...
    for (const filter of filters) {
      if (!appStateComposite.selectedCols?.includes(filter.column)) continue;
    ...
    if (pending && appStateComposite.selectedCols?.includes(pending.column)) {
...
class DataChart {
  ...
  updateDataMulti(dataObj, columns) {
    ...
    const seriesList = columns.filter((colName) => {
      ...
      const yValues = seriesData ? seriesData.y : dataObj.values?.[colName] ?? null;
      const xValues = seriesData ? seriesData.x : dataObj.ts ?? null;
      const points = [];
      ...
      const wantsColorBy = !!appStateComposite.selectedColorColumn && Array.isArray(seriesColors) && seriesColors.length === points.length;
      if (wantsColorBy) {
        return [{ __colorCandidate: true, colName, idx, visible, points, colorValues: seriesColors }];
      }
      const numColIdx = appStateComposite.numericCols.indexOf(colName);
    ...
    const colorColumn = appStateComposite.selectedColorColumn;
...
`;
  ...
  exportSVGDrawings(viewWidth, viewHeight) {
    ...
    const scaleX = viewWidth / (baseW);
    const scaleY = viewHeight / (baseH);
...
`;
  ...
  _initDrawingOverlay() {
    ...
    this._overlays = new ChartOverlays({
      ...
      getPendingAdaptivePoint: () => appStateComposite.pendingAdaptivePoint
    });
  ...
  _renderAdaptiveFilterLinesToCtx(ctx, scale) {
    const filters = Array.isArray(appStateComposite.adaptiveLineFilters) ? appStateComposite.adaptiveLineFilters : [];
    const pending = appStateComposite.pendingAdaptivePoint;
    if (filters.length === 0 && !pending) return;
    if (!this._container) return;
    const visibleCols = new Set(appStateComposite.selectedCols || []);
    const xMin = Number(this._xMin);
    const xMax = Number(this._xMax);
    const yRange = this.getYRange();
    if (!Number.isFinite(xMin) || !Number.isFinite(xMax) || !(xMax > xMin) || !yRange) return;
