...
import { g as getEl, i as isTemporalDtype, f as formatTimestamp, a as formatTwoDecimals, S as SERIES_COLORS, b as buildAdaptiveLineFiltersForQuery, c as appState, d as downloadBlob, e as downloadUrl, _ as __vitePreload, h as escapeHtml, j as fetchFft, k as fetchScatterPoints, l as isRangeOutsideDataset, m as fetchScatterCorrelations, n as defaultGpuPowerPreference, o as createEmptyStateController, r as requestGpuAdapter } from './frequency-DsOq7zgH.js';
...
const collectColumnRangeFilters = () => Object.entries(appState.columnRanges || {}).map(([column, range]) => {
...
function renderScatterExportToCanvas(canvas) {
  ...
  ctx.restore();
  const showContinuousLegend = controls.renderMode === "density" || controls.selectedColorColumn && !buildCategoricalColorGroups(appState.scatter.colorLabels) && Number.isFinite(appState.scatter.colorMin) && Number.isFinite(appState.scatter.colorMax) && appState.scatter.colorMax > appState.scatter.colorMin;
...
function buildDensityTooltipCache(series, controls, container) {
  ...
  const key = [
    appState.scatter.view.xMin,
    appState.scatter.view.xMax,
    appState.scatter.view.yMin,
    appState.scatter.view.yMax,
    metrics.plotWidth,
    metrics.plotHeight,
    binSize,
...
function updateColorbarUI() {
  if (appState.scatter.activeView !== "plot") {
    setColorbarVisible(false);
    return;
  }
  const ctl = currentControls();
  const isDensity = ctl.renderMode === "density";
  const hasContinuousColor = !!ctl.selectedColorColumn && Array.isArray(appState.scatter.colorValues) && appState.scatter.colorValues.length > 0 && Number.isFinite(appState.scatter.colorMin) && Number.isFinite(appState.scatter.colorMax) && appState.scatter.colorMax > appState.scatter.colorMin;
...
function buildOption(points, container) {
  ...
  const xSpan = Math.max(1, appState.scatter.view.xMax - appState.scatter.view.xMin);
  const ySpan = Math.max(1, appState.scatter.view.yMax - appState.scatter.view.yMin);
  const xTickFormatter = isTemporalColumn(ctl.x, appState.scatter.columnTypes) ? (v) => formatTimestamp(v, xSpan) : (v) => formatTwoDecimals(v);
  const yTickFormatter = isTemporalColumn(ctl.y, appState.scatter.columnTypes) ? (v) => formatTimestamp(v, ySpan) : (v) => formatTwoDecimals(v);
...
function updateBinnedReadout() {
  if (!appState.scatter.chart || appState.scatter.points.length === 0) {
  ...
  if (text !== appState.scatter.lastBinnedText) {
    appState.scatter.lastBinnedText = text;
...
function initSelectionZoom(container) {
  ...
  const hideSelectionBox = () => {
    if (appState.scatter.selectionBox) appState.scatter.selectionBox.style.display = "none";
  };
  ...
  container.addEventListener("dblclick", (ev) => {
    ...
    if (appState.scatter.zoomHistory.length > 0) {
      applyView(appState.scatter.zoomHistory.pop(), false);
      return;
    }
    resetView(false);
  });
...
function collectOverviewColumns() {
  ...
  for (const column of appState.scatter.metadata?.numeric_columns || []) {
    push(column);
    if (columns.length >= MATRIX_MAX_COLUMNS) break;
  }
...
async function refreshCorrelationsAndSuggestions() {
  ...
  if (!xSelect || !ySelect) return;
  const meta = appState.scatter.metadata;
  ...
  if (colorSelect) {
    const colorOptions = [""].concat(
      (appState.scatter.metadata?.columns || []).map((col) => String(col?.name || "")).filter(Boolean)
    );
  ...
  appState.scatter.correlationsByColumn = /* @__PURE__ */ new Map();
...
async function renderScatter() {
  ...
  try {
    ...
    if (!appState.scatter.chart) {
      ...
      if (powerPreference) ;
      appState.scatter.chart = await Ad(container, chartOptions);
...
//# sourceMappingURL=scatter-YST55Tu_.js.map
