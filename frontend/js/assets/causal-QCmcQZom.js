import { t as toast, q as fetchCausalGraph, _ as __vitePreload } from './frequency-DsOq7zgH.js';
...
function saveRun(links, columns, params, label) {
  const run = {
    id: generateId(),
    label: `${params.method} τ=${params.tauMax} α=${params.alpha}`,
    timestamp: Date.now(),
    ...params,
    columns: [...columns],
    links: links.map((l) => ({ ...l }))
...
let _compareRunAId = null;
let _compareRunBId = null;
...
const causalComparison = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
    __proto__: null,
    clearAllRuns,
    deleteRun,
...
let _chartInitPromise = null;
let _currentColumns = [];
let _currentLinks = [];
let _currentTauMax = 0;
let _nodeLabels = /* @__PURE__ */ new Map();
let _chipColors = /* @__PURE__ */ new Map();
let _nodeAttrs = /* @__PURE__ */ new Map();
let _pairAttrs = /* @__PURE__ */ new Map();
let _nodePositions = /* @__PURE__ */ new Map();
let _selectedColumns = /* @__PURE__ */ new Set();
let _editTarget = null;
let _addEdgeMode = false;
let _addEdgeFirst = null;
let _activePopover = null;
let _edgeEditDraft = null;
let _draftSeq = 0;
const METHOD_PC_STAGE = /* @__PURE__ */ new Set(["pcmci", "pcmciplus", "lpcmci"]);
...
function edgeLabelText(group, compact) {
  ...
  const secondLine = compact ? `${edgeDirectionGlyph(group)} τ${compactLagSummary(group)}` : `τ${compactLagSummary(group)} · ${group.connections.length} links`;
  return `${firstLine}
${secondLine}`;
}
...
function renderColumnChips(deps, columnsBar) {
  ...
  selectAllBtn.addEventListener("click", () => {
    const tauInputEl = document.getElementById("causal-tau-max");
...
function nodeTooltip(col, selfLoops) {
  ...
  return `<b>${escH(label)}</b><br/>Column: ${escH(col)}<br/>dtype: ${escH(dtype)}<br/>Incoming raw links: ${incoming} · Outgoing raw links: ${outgoing}` + (self ? ` · Self-links: ${self}` : "") + `<br/>Attributes: ${attrKeys.length}<br/><span style="font-size:10px;color:#788bae;">Drag moves only this node. Double-click rename. Right-click edit/delete.</span>`;
}
...
const causalPage = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
    __proto__: null,
    initCausalPage
}, Symbol.toStringTag, { value: 'Module' }));

export { causalPage as a, causalComparison as c };
//# sourceMappingURL=causal-QCmcQZom.js.map
