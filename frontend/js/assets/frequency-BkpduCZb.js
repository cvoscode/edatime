...
const appStateComposite = {
  ...
  set rollingWindow(v) {
    analyticsState.rollingWindow = v;
  },
  get rollingBands() {
    return analyticsState.rollingBands;
  },
  set rollingBands(v) {
    analyticsState.rollingBands = v;
  },
  get anomalyEnabled() {
  ...
  set currentStart(v) {
    chartState.currentStart = v;
  },
  get currentEnd() {
    return chartState.currentEnd;
  },
  set currentEnd(v) {
    chartState.currentEnd = v;
  },
  get initialView() {
    return chartState.initialView;
  },
  set initialView(v) {
    chartState.initialView = v;
  },
  get zoomHistory() {
    return chartState.zoomHistory;
  },
  set zoomHistory(v) {
    chartState.zoomHistory = v;
  },
  get chartText() {
    return chartState.chartText;
  },
  set chartText(v) {
    chartState.chartText = v;
  },
...
function getSeriesColor(column, fallbackIndex = 0) {
  ...
  const custom = normalizeSeriesColor(appStateComposite.seriesColors?.[name]);
...
function setSeriesColor(column, value) {
  ...
  appStateComposite.seriesColors = {
    ...appStateComposite.seriesColors || {},
    [name]: normalized
  };
  return normalized;
...
function sanitizeSelectedColumns() {
  ...
  appStateComposite.selectedCols = (appStateComposite.selectedCols || []).filter((col) => {
...
function ensureRangeStateFromData(dataObj) {
  for (const col of appStateComposite.selectedCols) {
    ...
    if (!appStateComposite.columnRanges[col]) {
      const bounds = computeBounds(values);
      if (!bounds) continue;
      appStateComposite.columnRanges[col] = { from: bounds.min, to: bounds.max };
    }
  }
}
...
function passesAdaptiveLineFilters(tsMs, valuesByColumn) {
  const filters = Array.isArray(appStateComposite.adaptiveLineFilters) ? appStateComposite.adaptiveLineFilters : [];
...
function buildAdaptiveLineFiltersForQuery() {
  return (appStateComposite.adaptiveLineFilters || []).map((filter) => ({
...
function applyColumnRanges(dataObj) {
  ...
  const lineFilters = Array.isArray(appStateComposite.adaptiveLineFilters) ? appStateComposite.adaptiveLineFilters : [];
  const neededColumns = lineFilters.length > 0 ? [.../* @__PURE__ */ new Set([...appStateComposite.selectedCols || [], ...lineFilters.map((f) => f.column)])] : [];
  for (const col of appStateComposite.selectedCols) {
...
${String(date.getHours()).padStart(2, "0")}:${String(date.getMinutes()).padStart(2, "0")}`;
  ...
  if (appStateComposite.metadata) {
    for (const column of appStateComposite.metadata.numeric_columns) {
  ...
  document.getElementById("spectrogram-compute-btn")?.addEventListener("click", async () => {
    ...
    try {
      ...
      const startMs = appStateComposite.currentStart;
      const endMs = appStateComposite.currentEnd;
  ...
  window.addEventListener("edatime:page-change", (event) => {
    ...
    if (detail?.page === "spectrogram" && appStateComposite.metadata) {
...
const spectrogramPage = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  initSpectrogramPage
}, Symbol.toStringTag, { value: 'Module' }));
...
function fftColumns() {
  return getNumericColumns(appStateComposite.metadata);
}
...
function renderChips() {
  ...
  if (!bar || !appStateComposite.metadata) return;
...
async function initFftPage(deps) {
  ...
  const populateChips = () => {
    if (appStateComposite.metadata) renderChips();
  };
  populateChips();
  ...
  document.getElementById("fft-filter-apply-btn")?.addEventListener("click", async () => {
    ...
    if (!filterType || filterType === "none") {
      if (appStateComposite.spectralFilterPreview) {
        appStateComposite.spectralFilterPreview = null;
        appStateComposite.chart?.requestOverlayRender?.();
        deps.renderTimeseries();
      }
      return;
    }
    const column = fftTraces[0]?.column || appStateComposite.selectedCols[0];
    if (!column) {
    ...
    try {
      const start = appStateComposite.currentStart;
      const end = appStateComposite.currentEnd;
      ...
      appStateComposite.spectralFilterPreview = {
        column: data.column,
        ts: data.ts,
        values: data.values,
        filterType,
...
const fftPage = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  initFftPage
}, Symbol.toStringTag, { value: 'Module' }));
...
const heatmapPage = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  initHeatmapPage
}, Symbol.toStringTag, { value: 'Module' }));
