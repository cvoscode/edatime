import { e as createSignal, i as insert, h as createComponent, w as uploadStore, b as createRenderEffect, n as className, S as Show, t as template, u as use, k as delegateEvents, f as createMemo, m as memo, s as setStyleProperty, j as setAttribute, F as For, r as useNavigate, o as onMount, g as uiStore, v as datasetStore } from './index.C65iRSEf.js';
import { b as fetchSampleETTm2, u as uploadPreview$1, c as uploadIngest, d as fetchMetadata } from './api.CIKlHxkp.js';
import { S as SwitchToggle } from './Dropdown.r_zpDvuI.js';
...
const ColumnProfileGrid = (props) => {
  ...
  const isDatetimeType = (dtype) => {
    return dtype.includes("datetime") || dtype.includes("date");
  };
  const formatMinMax = (value, dtype) => {
  ...
  return (() => {
    ...
    insert(_el$18, createComponent(For, {
      ...
      children: ({
        ...
        insert(_el$26, () => formatMinMax(profile.min, profile.dtype));
        insert(_el$27, () => formatMinMax(profile.max, profile.dtype));
        insert(_el$29, (() => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      datasetStore.setColumns(freshMetadata.column_profiles.map((cp) => ({
        name: cp.name,
        type: cp.dtype.includes("int") || cp.dtype.includes("float") || cp.dtype.includes("double") ? "numeric" : cp.dtype.includes("datetime") || cp.dtype.includes("date") ? "datetime" : "categorical",
        min: cp.min ?? void 0,
        max: cp.max ?? void 0,
...
//# sourceMappingURL=UploadPage.C8frZ1EX.js.map
