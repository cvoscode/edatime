...
function correlationColor(value, scaleName = "rdbu") {
  if (value === null) return "var(--color-surface)";
  const v = Math.max(-1, Math.min(1, value));
  const scale = getColorScale(scaleName);
  return scale(v).css();
}
function getColorScaleGradientCSS(scaleName, steps = 7) {
...
function getColorPalette(scaleName, count = 6) {
  const def = COLORMAPS[scaleName];
...
export { COLORMAPS as C, getColorScaleGradientCSS as a, getContrastTextColor as b, correlationColor as c, buildCategoricalColorGroups as d, getCategoryColor as e, getColorPalette as g, sampleGradient as s };
//# sourceMappingURL=colorScale.CLTVXB24.js.map
